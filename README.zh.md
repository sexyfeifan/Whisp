...
- **检查更新** — 内置更新检测，查询 GitHub Releases 获取新版本，显示更新日志并提供直接下载链接。
- **设置项描述** — 设置页的开关选项现在显示说明文字，提升可读性。
- **API Key 持久化修复** — 修复 API Key 未写入磁盘导致每次重启都提示「未配置」的关键 bug。
- **UTF-8 安全** — 修复截断多字节（中日韩）错误消息时可能崩溃的问题。
- **性能优化** — 减少转写重试循环中的内存分配。

## v2.4 更新亮点

- **静音自动停止** — 可配置的静音检测，说完话自动停止录音，无需再按快捷键。
- **Whisper 提示词** — 添加上下文提示词，提高专业词汇的转写准确率。
- **静音阈值调节** — 调整静音检测灵敏度以适应不同环境。
- **日语界面** — 新增完整日语语言支持，与中文和英文并列。
- **麦克风选择** — 在设置中选择首选麦克风设备。
- **浮窗位置记忆** — 波形浮窗跨会话记住上次位置。
- **开机自启** — 可选择登录时自动启动 Whisp。
- **历史分页与批量操作** — 分页加载历史记录，支持批量删除和清空。
- **多服务商自动识别** — 自动识别 OpenAI、Groq、Fireworks、Deepgram、Google Cloud 或自定义服务商。
- **系统托盘图标** — 支持 macOS 深色/浅色模式的自定义模板图标。
...
2. 说话
3. 再按一次停止（或等待静音自动停止）— 文字瞬间转写并粘贴

## 特性

- **一个快捷键** — 全局热键启停录音，无需操作界面。
- **自动粘贴** — 转写文字直达光标位置，无需手动复制。
- **静音自动停止** — 可配置静音时长，说完自动停止录音。
- **模型预置 + 自定义** — 内置主流模型名称，也可手动输入任意模型名。
- **模型说明面板** — 在设置页点击 `Model Guide` 可查看说明并一键选择。
- **波形浮窗** — 录音时显示极简的置顶波形动画，位置跨会话记忆。
- **历史记录** — 支持状态展示、搜索筛选、分页加载、批量删除与带音频重试的本地历史记录。
- **多语言界面** — 支持简体中文、英语和日语。
- **系统托盘** — 安静地驻留后台，支持开机自启。

## 下载

从 [Releases](https://github.com/sexyfeifan/Whisp/releases/latest) 下载最新 macOS DMG 安装包：
