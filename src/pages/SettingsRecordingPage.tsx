...
function SettingsRecordingContent(app: AppState) {
  const {
...
export function SettingsRecordingPage(app: AppState) {
  ...
  if (embedded) {
    return (
      <motion.div key="settingsRecording" variants={viewVariants} initial="initial" animate="animate" exit="exit" transition={{ type: "spring", stiffness: 300, damping: 30, mass: 0.8 }} className="p-6">
        <SettingsRecordingContent {...app} />
      </motion.div>
    );
  }
