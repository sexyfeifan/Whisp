...
- **实时流式转写**：录音过程中在桌面弹出胶囊形浮窗，实时显示转写文字，说完即见
- **自动粘贴**：转写完成后自动复制到剪贴板并粘贴到当前应用（Linux 支持 xdotool/ydotool/wtype 多级 fallback）
...
- **录音摘要**：一键为转写内容生成 AI 摘要，快速回顾要点
- **独立 API 配置**：摘要可配置不同于转写的 API 端点和密钥，灵活选择模型
- **自动回退**：未配置摘要 API 时自动使用转写 API，零配置也能用
...
- **音频播放**：内置音频播放器，支持进度条拖动、播放/暂停控制，播放时转写文字同步高亮
- **AI 摘要**：对转写记录一键生成 AI 摘要，快速回顾录音要点
...
- **系统托盘**：最小化到系统托盘，显示紫色光球呼吸动画图标，支持最近转写记录快速访问
- **开机自启**：可选登录时自动启动
...
> **v2.12 最新更新**：实时流式转写胶囊浮窗、紫色光球托盘动画、音频播放进度条与文字同步高亮、AI 摘要独立配置、Linux 自动粘贴多级 fallback、流式转写速度优化。
...
- **独立 API 配置**：可单独配置摘要用的 API 地址、密钥和模型名
- **自动回退**：未配置时自动复用转写的 API 配置
- **触发方式**：在历史记录中点击摘要按钮即可生成

### 录音设置

- **实时流式转写**：录音过程中实时显示转写结果，支持自定义轮询间隔和音频分片时长
